//
//  OSCAPressReleaseTests.swift
//  OSCAPressReleasesTests
//
...
///  | pygmentize -g
///  ```
class OSCAPressReleaseTests: OSCAParseClassObjectTestSuite<OSCAPressRelease> {
  override open func makeSpecificObject() -> OSCAPressRelease? {
    nil
  ...
  /// The file name has to match with the test class name: `OSCAPressReleaseTests.json`
  override func testJSONDataAvailable() throws -> Void {
    try super.testJSONDataAvailable()
  }// end override func testJSONDataAvailable
#if DEBUG
  /// Is it possible to deserialize `JSON` scheme example data to an array  `OSCAPressRelease` 's?
  override func testDecodingJSONData() throws -> Void {
    try super.testDecodingJSONData()
  }// end override func testDecodingJSONData
#endif
  /// is it possible to fetch up to 1000 `OSCAPressRelease` objects in an array from network?
...
}// end class OSCAPressReleaseTests
#endif
